...
def _iter_files(root: Path, suffix: str) -> list[Path]:
    """All files with suffix under root, skipping junk dirs."""
    out: list[Path] = []
...
def _py_files_for_mcp_count(root: Path) -> list[Path]:
    """Python files likely to register MCP tools."""
...
def main() -> None:
    ...
    name, version, req_py = _parse_name_version_requires(pyproject)
    folder = root.name
    ...
    fastmcp_inst: str | None = None
    with contextlib.suppress(importlib.metadata.PackageNotFoundError):
        fastmcp_inst = importlib.metadata.version("fastmcp")
    ...
    lines = [
        ...
        f"pyproject: {name or '(no name)'} {version or ''} | Python {req_py or '?'}",
        f"FastMCP (declared): {fastmcp_decl or '—'}",
        f"FastMCP (installed): {fastmcp_inst or '—'}",
    ]

    md_msg = f"Markdown: {len(md_all)} total | active: {md_active}"
    if md_archived:
        md_msg += f" | archived: {md_archived}"
    lines.append(md_msg)

    lines.extend(
        [
            f"tools/*.py: {n_tools_py} | scripts/*.py: {n_scripts_py}",
            f"MCP tools (approx.): {mcp_tool_decorators}",
