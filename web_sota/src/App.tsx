  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh" }}>
      ...
      >
        ...
        <nav style={{ color: "var(--muted)", fontSize: 14 }}>
          <div style={{ color: "var(--text)", marginBottom: 8 }}>Home</div>
          <div>Tools</div>
          <div>Settings</div>
          <div>Help</div>
        </nav>
      ...
      <main style={{ flex: 1, padding: "2rem" }}>
        <header style={{ marginBottom: "2rem" }}>
          ...
          <p style={{ color: "var(--muted)", margin: 0, maxWidth: 560 }}>
            FastMCP 3.1 + OpenManus (FOSS). Ports <strong>10769</strong> (UI) /{" "}
            <strong>10768</strong> (API).
          </p>
        </header>
        ...
        {err && (
          ...
          >
            API unreachable: {err}. Start backend:{" "}
            <code>uv run python -m openmanus_mcp.run_api</code>
          </div>
        )}
        ...
        <footer style={{ marginTop: "3rem", color: "var(--muted)", fontSize: 13 }}>
          MCP tool: <code>openmanus_bridge</code> · v0.1 scaffold
        </footer>
      </main>
    </div>
  );
